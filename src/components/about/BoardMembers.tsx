        ...
        <div>
            <div className='md:w-[15.9269rem] md:h-[16.1431rem]  relative z-[1]  before:contents-[""] before:w-full before:h-full before:absolute before:bg-transparent before:border-[.125rem] before:border-[#C2A75A] before:right-[.4544rem] before:top-[.4544rem] before:z-[-1] before:opacity-50 before:pointer-events-none group' >
                <div className='overflow-hidden h-full w-full' >
                    <Image width={500} height={500} alt='event' src={image} className='w-full h-full object-cover group-hover:scale-105 transition-all duration-200 ease-linear' />
                </div>
            </div>
        </div>
        <div className='p-4 space-y-8' >
            ...
            <div>
                ...
                <div className={`pb-2 px-1 w-fit border-b border-primary-dark-gradient relative group overflow-hidden cursor-pointer`} onClick={() => handleSelect(index)} >
                    <div className='group-hover:transform group-hover:translate-y-[--1.5rem] transition-transform duration-200' >

                        <p className={`font-medium text-lg md:leading-6  text-primary-dark-gradient`} >Real All</p>
                    </div>
                    <div className='absolute left-1 right-0 -bottom-5 group-hover:transform group-hover:translate-y-[--1.75rem] transition-transform duration-200'>
...
export const BoardMembers = () => {
    ...
    return (
        <div className='py-[6.25rem] bg-on-surface-bright-white' >
            ...
            <MaxWidthWrapper>
                <div className='space-y-[3rem]' >
                    <div className='space-y-6 w-[25.5rem]   bg-transparent border-b-[.0625rem] border-primary-dark-gradient/50  pb-6'>
                        <p className='font-diamend text-[2.25rem] leading-[3rem]  text-primary-on-light' >Board Members</p>

                    </div>
                    <div className='flex gap-[4rem]' >
                        <div className='flex-1/2 space-y-[6rem]' >
                            {
                        ...
                        <div className='flex-1/2 space-y-[6rem] ' >
                            {
                                board.slice(3, 6).map((item, idx) => {
                                    return <Card key={idx} image={item.image} designation={item.designation} name={item.name} desc={item.desc} link={item.link} />
                                })
                            }
                        </div>
                    </div>
                </div>
                <div className='pt-[6.25rem] w-fit mx-auto' >
                    <JoinText text={"View All Ambassador Club’s Team"} link={"/"} />
                </div>
            </MaxWidthWrapper>
        </div>
    )
